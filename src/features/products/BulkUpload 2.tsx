  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        Papa.parse(file, {
          ...
          complete: (results: Papa.ParseResult<CSVRow>) => {
            if (results.errors.length) {
              ...
              reject(`Error en la fila ${firstError.row}: ${firstError.message}`);
            } else {
              const validData = results.data.filter(row => 
                row.SKU && row.Nombre && row.Precio && row.Categoria
              );
      ...
      const products: Product[] = results.map((row, index) => ({
        ...
        price: parseFloat(row.Precio.replace(/[^\d.-]/g, '')) || 0,
        category: row.Categoria.trim(),
        imageUrl: (row['URL de Imagen'] || '').trim(),
        description: `Producto importado: ${row.Nombre.trim()}`,
        pricePerUnit: '',
        points: '',
        origin: '',
        barcode: '',

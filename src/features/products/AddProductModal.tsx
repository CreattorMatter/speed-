...
import { X, FileText, Upload, Smartphone, Plus, AlertCircle } from 'lucide-react';
...
const AddProductModal: React.FC<AddProductModalProps> = ({ isOpen, onClose, onAddProduct }) => {
  ...
  const renderMethodContent = () => {
    switch (selectedMethod) {
      case 'manual':
        return <ManualForm onSubmit={(product: Product) => onAddProduct(product)} onBack={() => setSelectedMethod(null)} />;
      case 'bulk':
        return <BulkUpload onSubmit={(products: Product[]) => onAddProduct(products)} onBack={() => setSelectedMethod(null)} />;
      case 'mobile':
        return <MobileScanner onSubmit={(product: Product) => onAddProduct(product)} onBack={() => setSelectedMethod(null)} />;
      default:
        return (

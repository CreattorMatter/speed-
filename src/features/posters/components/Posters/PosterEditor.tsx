...
import { FamilyV3, TemplateV3 } from '../../../builderV3/types';
...
export const PosterEditor: React.FC<PosterEditorProps> = ({
  ...
  // Estado para familias y plantillas del Builder V3
  ...
  const [builderTemplate, setBuilderTemplate] = useState<TemplateV3 | null>(null);
  ...
  // Efecto para cargar familias al montar el componente
  useEffect(() => {
  ...
  // Efecto para filtrar plantillas cuando cambia la familia seleccionada
  useEffect(() => {
    if (selectedFamily) {
      // En una implementación real, las plantillas vendrían de la familia
      // o se haría otra llamada al servicio: getTemplatesByFamily(selectedFamily.id)
      setTemplates(selectedFamily.templates || []);
  ...
  // Handlers para el nuevo selector
  const handleFamilyChange = (family: FamilyV3 | null) => {
    setSelectedFamily(family);
  };

  const handleTemplateChange = (template: TemplateV3 | null) => {
            ...
            <div className="flex flex-col lg:grid lg:grid-cols-10 gap-4 sm:gap-6 h-full">
              ...
              <div className="w-full lg:col-span-7 order-1 lg:order-2 min-h-[400px] sm:min-h-[500px] lg:min-h-[600px]">
                <PreviewArea
                  ...
                  selectedBuilderTemplate={builderTemplate}
                  builderComponents={builderTemplate?.defaultComponents || []}
                />
              </div>
            </div>
